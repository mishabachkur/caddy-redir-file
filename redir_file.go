...
type Middleware struct {
	...
	// PreserveQuery determines whether to keep query parameters during redirection
...
func (m Middleware) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	if newPath, ok := m.redirects[r.URL.Path]; ok {
		if m.PreserveQuery && r.URL.RawQuery != "" {
			newPath = fmt.Sprintf("%s?%s", newPath, r.URL.RawQuery)
		}
